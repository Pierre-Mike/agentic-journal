...
- [x] 1. Author spec scaffolding — design.md and tasks.md alongside the already-written proposal.md
  - agent: main
  - depends: []
...
- [x] 2. RED gate + colocated bun:test — shape-gate asserting three-agent invariants
  - agent: main
  - depends: []
...
- [x] 3. Create spec-tester agent — Sonnet; authors RED state; scoped to specs/active/<id>/ + gate paths
  - agent: main
  - depends: [1, 2]
  - file_targets: [.claude/agents/spec-tester.md]
  - boundary: [.claude/agents/spec-tester.md]
- [x] 4. Create spec-judge agent — Opus; 4-item rubric verbatim; touches .gate-frozen on PASS; writes blocker.md on 3-strike FAIL
  - agent: main
  - depends: [1, 2]
  - file_targets: [.claude/agents/spec-judge.md]
  - boundary: [.claude/agents/spec-judge.md]
- [x] 5. Create spec-implementer agent — Sonnet; gate paths frozen (hook-enforced); executes Steps 6–10
  - agent: main
  - depends: [1, 2]
  - file_targets: [.claude/agents/spec-implementer.md]
  - boundary: [.claude/agents/spec-implementer.md]
- [x] 6. Extend enforce hook — frozen-gate check + fixture tests + settings.json permission rules
  - agent: main
  - depends: [3, 4, 5]
...
- [x] 7. Rewrite SKILL.md orchestration — Step 2.5 three-dispatch chain + Step 10 escalated variant
  - agent: main
  - depends: [3, 4, 5]
  - file_targets: [.claude/skills/do/SKILL.md]
  - boundary: [.claude/skills/do/SKILL.md]
- [x] 8. Verify — `bun run tasks:verify` green end-to-end
  - agent: main
