...
- [x] 1. Implement `readingTime` and `WPM` in `src/lib/reading-time.ts` so the gate passes
  - agent: main
  - depends: []
  - file_targets: [src/lib/reading-time.ts]
- [x] 2. Wire into post route: call `readingTime(post.body)` and append `· {minutes} min read` to the existing meta line
  - agent: main
  - depends: [1]
